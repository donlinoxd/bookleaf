...
import {
  ActivityIndicator, FlatList, Modal, ScrollView, StatusBar,
...
export default function ClientHomeScreen() {
  ...
  const clearFilters = () => {
    setPendingType(''); setPendingYearFrom(''); setPendingYearTo(''); setPendingLanguage('');
    setFilterType(''); setFilterYearFrom(''); setFilterYearTo(''); setFilterLanguage('');
    setFilterVisible(false);
  };

  const openFilter = () => {
    setPendingType(filterType); setPendingYearFrom(filterYearFrom);
    setPendingYearTo(filterYearTo); setPendingLanguage(filterLanguage);
  ...
  const BookCard = ({ item }: { item: BookResult }) => (
    ...
    >
      ...
      <View className="flex-1 ml-4">
        <Text className="text-base font-bold text-[#1C2B1E] leading-5" numberOfLines={2}>{item.title}</Text>
        <Text className="text-sm font-medium text-[#5A7A5E] mt-1">{item.author}</Text>
        <View className="flex-row flex-wrap gap-1.5 mt-2">
          {item.genre && <View className="bg-mint rounded-md px-2 py-0.5"><Text className="text-xs font-semibold text-brand">{item.genre}</Text></View>}
          {item.year && <View className="bg-mint rounded-md px-2 py-0.5"><Text className="text-xs font-semibold text-brand">{item.year}</Text></View>}
          {item.language && <View className="bg-mint rounded-md px-2 py-0.5"><Text className="text-xs font-semibold text-brand">{item.language}</Text></View>}
        </View>
  ...
  const HorizontalBookCard = ({ item }: { item: BookResult }) => (
    <TouchableOpacity
    ...
    <View className="flex-1 bg-bio">
      ...
      <View className="bg-brand px-5 pb-6 pt-[52px] rounded-b-[28px]">
        ...
        <View className="flex-row gap-2">
          <View className="flex-1 flex-row bg-white rounded-2xl overflow-hidden"
          ...
          ListHeaderComponent={
            books.length > 0 ? (
              <View className="flex-row items-center justify-between mb-3">
                <Text className="text-sm text-slate-500 font-medium">{books.length} result{books.length !== 1 ? 's' : ''}</Text>
                {hasActiveFilters && (
            ...
            <>
              ...
              {recent.length === 0 && popular.length === 0 && (
                <View className="items-center pt-12 px-8">
                  ...
                  <Text className="text-sm text-[#7A9A7E] text-center leading-5">Search the catalog by title, author, call number, or publisher</Text>
                </View>
              )}
            </>
          )}
