      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          <Section title="Collection Overview" badge="CHED">
            ...
            <View style={{ marginTop: 12, backgroundColor: '#E2EFE0', borderRadius: 10, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            {condition.length === 0 ? (
              ...
              <View style={{ flexDirection: 'row', gap: 10 }}>
                {condition.map((row) => {
                  ...
                  return (
                    <View key={row.condition} style={{
                      flex: 1, borderRadius: 12, padding: 14, alignItems: 'center',
                      backgroundColor: c.bg,
                    }}>
                      <Text style={{ fontSize: 22, fontWeight: '800', color: c.text }}>{row.copies}</Text>
                      <Text style={{ fontSize: 11, fontWeight: '700', color: c.text, marginTop: 3, textTransform: 'capitalize' }}>
                        {row.condition}
                      </Text>
                    </View>
                  );
      ...
      {cols.map((col, i) => (
        <Text
          ...
          style={{
            flex: widths[i] === 'flex' ? 1 : undefined,
            width: widths[i] !== 'flex' ? widths[i] : undefined,
            fontSize: 12, color: i === 0 ? '#1C2B1E' : '#3A7A45',
...
function MiniStat({ label, value, accent, bg }: { label: string; value: number; accent: string; bg: string }) {
  return (
    <View style={{
      borderRadius: 12, padding: 12, alignItems: 'center', minWidth: 90,
      backgroundColor: bg, flexGrow: 1,
    }}>
