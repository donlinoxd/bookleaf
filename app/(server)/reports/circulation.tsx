            ...
            {monthly.length === 0 ? (
              ...
              <View style={{ gap: 6 }}>
                <View style={{ flexDirection: 'row', paddingHorizontal: 4, marginBottom: 2 }}>
                  <Text style={{ width: 72, fontSize: 10, fontWeight: '700', color: '#64748B', textTransform: 'uppercase' }}>Month</Text>
                  <Text style={{ flex: 1, fontSize: 10, fontWeight: '700', color: BRAND, textTransform: 'uppercase' }}>Borrowed</Text>
                  <Text style={{ flex: 1, fontSize: 10, fontWeight: '700', color: LEAF, textTransform: 'uppercase' }}>Returned</Text>
                </View>
                {monthly.map((row, i) => (
                  <View key={row.month} style={{
                    backgroundColor: i % 2 === 1 ? '#F8FAFC' : 'transparent',
                    borderRadius: 8, paddingHorizontal: 4, paddingVertical: 6, gap: 4,
                  }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <Text style={{ width: 72, fontSize: 11, color: '#64748B', fontWeight: '600' }}>{row.label}</Text>
                      <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        <View style={{ flex: row.borrows / maxBorrows, height: 10, backgroundColor: BRAND + 'CC', borderRadius: 3, maxWidth: '70%' }} />
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <View style={{ width: 72 }} />
                      <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        <View style={{ flex: row.returns / maxBorrows, height: 10, backgroundColor: LEAF + 'CC', borderRadius: 3, maxWidth: '70%' }} />
            ...
            {mostBorrowed.length === 0 ? (
              ...
              <>
                ...
                {mostBorrowed.map((row, i) => (
                  <View key={row.resource_id} style={{
                    flexDirection: 'row', paddingHorizontal: 10, paddingVertical: 9, alignItems: 'center',
                    backgroundColor: i % 2 === 1 ? '#F8FAFC' : 'transparent', borderRadius: 6,
                  }}>
                    <Text style={{ width: 28, fontSize: 11, color: '#94A3B8', fontWeight: '700' }}>{i + 1}</Text>
                    <View style={{ flex: 1, paddingRight: 8 }}>
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#1C2B1E' }} numberOfLines={1}>{row.title}</Text>
                      <Text style={{ fontSize: 11, color: '#7A9A7E', marginTop: 1 }} numberOfLines={1}>{row.author}</Text>
                    </View>
              ...
              <>
                ...
                {topBorrowers.map((row, i) => (
                  <View key={row.user_id} style={{
                    ...
                    <Text style={{ width: 28, fontSize: 11, color: '#94A3B8', fontWeight: '700' }}>{i + 1}</Text>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 12, fontWeight: '700', color: '#1C2B1E' }} numberOfLines={1}>{row.user_name}</Text>
                      <Text style={{ fontSize: 11, color: '#7A9A7E', marginTop: 1 }}>{row.user_id_number}</Text>
                    </View>
                    <Text style={{ width: 48, fontSize: 12, fontWeight: '700', color: BRAND, textAlign: 'center' }}>{row.total_borrows}</Text>
                ...
                {overdue.map((row, i) => {
                  ...
                  return (
                    <View key={row.id} style={{
                      flexDirection: 'row', paddingHorizontal: 10, paddingVertical: 9, alignItems: 'center',
                      backgroundColor: i % 2 === 1 ? '#FFF7ED' : 'transparent', borderRadius: 6,
                    }}>
                      <View style={{ flex: 1, paddingRight: 6 }}>
                        <Text style={{ fontSize: 12, fontWeight: '700', color: '#1C2B1E' }} numberOfLines={1}>{row.member_name}</Text>
                        <Text style={{ fontSize: 10, color: '#7A9A7E' }}>{row.member_id_number}</Text>
                      </View>
                      <Text style={{ flex: 1, fontSize: 11, color: '#1C2B1E', paddingRight: 6 }} numberOfLines={2}>{row.book_title}</Text>
                      ...
                      <View style={{ width: 44, alignItems: 'center' }}>
                        <View style={{ backgroundColor: '#FEE2E2', borderRadius: 6, paddingHorizontal: 5, paddingVertical: 2 }}>
