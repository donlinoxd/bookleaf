...
import type { GateDirection } from '@bookleaf/types'
...
export default function ClientGateScreen() {
    ...
    const gateMutation = useMutation(
        trpc.gate.log.mutationOptions({
            onSuccess: (data: any) => {
                showResult({ direction: data.direction, user_name: data.user_name })
            },
            onError: (e: unknown) => {
    ...
    const handleBarcodeScan = async ({ data }: { data: string }) => {
        ...
        gateMutation.mutate(undefined as any)
    }
