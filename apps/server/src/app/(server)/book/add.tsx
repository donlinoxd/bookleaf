...
import { IDENTIFIER_LABEL, MATERIAL_TYPES, MATERIAL_TYPE_META } from '../../../src/lib/materialTypes'
import { IsbnLookupService } from '../../../src/services/IsbnLookupService'
import { ResourceService } from '../../../src/services/ResourceService'
import { useAppStore } from '../../../src/store/appStore'
import { CallNumberType, MaterialType } from '@bookleaf/types'
...
export default function AddResourceScreen() {
    ...
    const handleSave = () => {
        ...
        if (!author.trim()) {
            Alert.alert('Error', 'Author / Creator is required')
            return
        }
        if (!institution) {
    ...
    return (
        <>
            <View className='flex-1 bg-bio'>
                ...
                <ScrollView contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 144 }}>
                    {/* Simple / RDA toggle */}
                    ...
                    <FormSection label={identifierLabel}>
                        ...
                        {coverUri && (
                            <View className='flex-row items-center gap-3 bg-mint rounded-xl p-3'>
                                <Image source={{ uri: coverUri }} className='w-12 h-16 rounded-lg' resizeMode='cover' />
                                <View className='flex-1'>
                                    <Text className='text-xs font-bold text-brand'>Cover found</Text>
                                    <Text className='text-xs text-[#5A7A5E] mt-0.5' numberOfLines={1}>
                                        {title || 'Untitled'}
                                    </Text>
                                    <TouchableOpacity className='mt-1.5' onPress={() => setCoverUri(null)}>
                    ...
                    <FormSection label='Details'>
                        ...
                        <View className='flex-row gap-2'>
                            ...
                            />
                            <TextInput
                                className='flex-[2] bg-white border border-mint rounded-xl px-4 py-3 text-sm text-[#1C2B1E]'
                                value={genre}
                        ...
                        <>
                            <FormSection label='Bibliographic Details'>
                                ...
                                <TextInput
                                    ...
                                    placeholder='Edition (e.g. 3rd ed.)'
                                    placeholderTextColor='#94A3B8'
                                />
                                <View className='flex-row gap-2'>
                                    <TextInput
                            ...
                            <FormSection label='Digital / Online'>
                                ...
                                {materialType === 'AUDIOVISUAL' && (
                                    <TextInput
                                        ...
                                        placeholder='Duration (e.g. 1h 23m)'
                                        placeholderTextColor='#94A3B8'
                                    />
                                )}
                            </FormSection>
                            ...
                            <FormSection label='RDA Descriptors'>
                                <TextInput
                                    ...
                                    placeholder='Content Type (e.g. text, spoken word)'
                                    placeholderTextColor='#94A3B8'
                                />
                                <TextInput
                                    className='bg-white border border-mint rounded-xl px-4 py-3 text-sm text-[#1C2B1E]'
                                    ...
                                    placeholder='Media Type (e.g. unmediated, audio)'
                                    placeholderTextColor='#94A3B8'
                                />
                                <TextInput
                                    className='bg-white border border-mint rounded-xl px-4 py-3 text-sm text-[#1C2B1E]'
                                    value={carrierType}
                                    onChangeText={setCarrierType}
                                    placeholder='Carrier Type (e.g. volume, audio disc)'
                                    placeholderTextColor='#94A3B8'
                                />
                            </FormSection>
                        </>
